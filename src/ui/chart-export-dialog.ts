...
export interface ChartExportDialogOptions {
  chartName: string;
...
export function showChartExportDialog(options: ChartExportDialogOptions): { destroy: () => void } {
  const overlay = createOverlay();

  const dialogTitleId = 'chart-export-dialog-title';
  ...
  title.textContent = t('chart_export_dialog.title');
  title.style.cssText = `
  ...
  if (options.versions.length > 0) {
    ...
    hintParagraph.textContent = t('chart_export_dialog.versions_hint');
    ...
    options.versions.forEach((version, index) => {
      ...
      checkbox.id = `chart-export-version-${version.id}`;
